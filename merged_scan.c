...
int main(void) {
    ...
    while (current_steps <= MAX_AZIMUTH_STEPS) {
        ...
        if (current_steps >= MAX_AZIMUTH_STEPS) {
            break;
        }

        spin_step_once(true, STEP_DELAY_US, &current_steps);
    ...
    while (current_steps > 0) {
        spin_step_once(false, STEP_DELAY_US, &current_steps);
        sleep_ms(STEP_SETTLE_MS);
    }
